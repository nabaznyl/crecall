...
### Maintenance & Cleanup
Use built-in cleanup utilities to remove stale build artifacts, caches, and aged backups.

Dry-run shell cleanup:
```bash
./scripts/cleanup.sh --dry-run --purge-pyc --purge-dist --keep-backups 1
```
Execute cleanup (30-day age filter, keep 2 most recent backups):
```bash
./scripts/cleanup.sh --execute --purge-pyc --purge-dist --keep-backups 2 --max-age-days 30
```
Advanced Python cleanup with JSON report:
```bash
python scripts/cleanup.py --dry-run --purge-pyc --list-large 20 --json-report cleanup_report.json
```
Note: This environment is Linux; the Windows path `C:\Users\Admin\Documents` is not accessible. Mount or sync that directory before attempting cross-platform cleanup operations.
...
- ✅ **Lifecycle Modernization**: FastAPI lifespan context replaces deprecated startup/shutdown events (no more deprecation warnings)
- ✅ **Timezone Migration**: All UTC timestamps now timezone-aware (`datetime.now(timezone.utc)`) eliminating naive ↔ aware warnings
- ✅ **Session Duplicate Handling**: Pre-check returns HTTP 409 instead of raw integrity traceback
- ✅ **Portable Encryption Fallback**: Graceful downgrade to plain bundle with `encryption_error` flag when crypto backend unavailable
- ✅ **Security Middleware Added**: Standard security headers + request ID injection; initial rate limiting scaffold (deterministic test harness forthcoming)
...
#### 2. **Python API (FastAPI)** (High-Performance Backend)
   ...
   - **Lifespan Context**: Using FastAPI's `lifespan` async context manager for startup/shutdown ensures clean resource initialization and eliminates deprecated `@app.on_event` usage.
   - **Security Middleware**: Injects security headers (CSP, X-Content-Type-Options, X-Frame-Options) and assigns request IDs for traceability; rate limiting layer present (pending production tuning).
...
- **API Documentation**: Coming soon with Python backend
 - **Lifecycle & Timezone Migration**: Implemented (v0.1.0d-7) – deprecated event handlers replaced by lifespan; all timestamps now UTC aware; see `TEST_RESULTS.md` for verification notes.
