...
from app.services.export_service import ExportService, SCHEMA_VERSION
...
async def test_export_import_roundtrip(db_session):
    ...
    await mem_service.create_memory(MemoryCreate(session_id="sessA", content="first memory", tags=["a"], category="demo", importance=1))
    await mem_service.create_memory(MemoryCreate(session_id="sessA", content="second memory", tags=["b"], category="demo", importance=0))
...
async def test_export_integrity_mismatch(db_session):
    mem_service = MemoryService(db_session)
    await mem_service.create_memory(MemoryCreate(session_id="sessB", content="only memory", tags=[], category=None, importance=0))
...
async def test_import_session_id_collision(db_session):
    mem_service = MemoryService(db_session)
    await mem_service.create_memory(MemoryCreate(session_id="sessC", content="m1", tags=[], category=None, importance=0))

    export_service = ExportService(db_session)
    data = await export_service.export_sessions(["sessC"])

    # Pre-create same session id to force collision
    await mem_service.create_memory(MemoryCreate(session_id="sessC", content="m2", tags=[], category=None, importance=1))
