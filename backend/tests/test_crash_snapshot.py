...
from app.services.crash_snapshot_service import get_crash_snapshot_service, SNAPSHOT_DIR
...
async def test_crash_snapshot_capture(db_session):
    ...
    await service.create_memory(MemoryCreate(session_id="snapA", content="snapshot test", importance=1))
...
async def test_crash_snapshot_periodic(db_session):
    service = MemoryService(db_session)
    await service.create_memory(MemoryCreate(session_id="snapB", content="periodic test", importance=0))
    snap_service = get_crash_snapshot_service()
    first = await snap_service.ensure_periodic(db_session, interval_minutes=0)  # force immediate
    assert first is not None and first.exists()
    second = await snap_service.ensure_periodic(db_session, interval_minutes=120)  # should not create
