...
class MetricsCollector:
    ...
    def prometheus(self) -> str:
        lines = ["# HELP crecall_uptime_seconds Uptime of the crecall backend","# TYPE crecall_uptime_seconds gauge", f"crecall_uptime_seconds {round(time.time() - self._started, 2)}"]
        # Counters
        for (name, labels), value in self._counters.items():
            label_str = ",".join([f"{k}='{v}'" for k,v in labels])
            lines.append(f"crecall_counter_total{{metric='{name}'{(','+label_str) if label_str else ''}}} {value}")
        # Latency summaries
        for (name, labels), samples in self._latency.items():
            if not samples:
                continue
