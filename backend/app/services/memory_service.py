...
class MemoryService:
    ...
    async def create_memory(self, memory_data: MemoryCreate) -> Memory:
        ...
        try:
            await broadcast_event(
                "memory.created", {"memory_id": memory.id, "session_id": memory.session_id}
            )
        except Exception:
            pass
    ...
    async def search_memories(
        ...
        result = await self.db.execute(
            stmt.order_by(Memory.created_at.desc()).limit(limit * 4)
        )  # fetch extra for ranking prune
    ...
    async def get_popular_tags(self, limit: int = 20) -> List[dict]:
        ...
        result = await self.db.execute(select(Memory.tags).where(Memory.tags.isnot(None)))
